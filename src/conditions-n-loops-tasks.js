...
function convertToRomanNumerals(num) {
  ...
  const arabicValues = [1, 4, 5, 9, 10, 40, 50, 90, 100, 39];

  if (num < 1 || num > 39) {
    return 'Number out of range (1-39)';
  }
  ...
  for (let i = romanSymbols.length - 1; i >= 0; i -= 1) {
    while (currentNum >= arabicValues[i]) {
      currentNum -= arabicValues[i];
